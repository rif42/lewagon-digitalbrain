...
- Internal site (Cloudflare Pages `lewagon-digitalbrain`, Access-gated via Email OTP): `quartz/quartz.config.internal.yaml` is the full vault — `explicit-publish` off, explorer/search/graph on, `ignorePatterns` only `private`/`templates`/`.obsidian`. The `build-internal` job in `deploy.yml` copies it over `quartz.config.yaml` at build time (the CLI hardcodes that path, no `--config` flag) and builds to `quartz/public-internal` (gitignored), then `wrangler pages deploy` uploads it. Needs repo secrets `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`. Trigger = push to `main`, same as public. Note: repo stays public, so Access gates the rendered internal site only — notes remain readable via git.
...
- `https://thebaliguideline.com/events` — **curated DJ-headliner calendar** (server-rendered Next.js, easy). 26 events, each with detail page at `https://thebaliguideline.com/event/<area>/<slug>` carrying exact Date & Time (WITA), venue, ticket link (megatix/savaya.com). Best source for Savaya/Zumana international acts. Check "Other Events You Might Like" for cross-links.
- `https://balisquad.com/event` — **nomad/community layer** (easy, ~135 events). Comedy nights, beach days, suppers, game nights, sport. Detail pages at `https://balisquad.com/event/<slug>-<id>`.
- `https://www.meetup.com/find/id--canggu/` — **expat/nomad meetups** (easy, server-rendered). Craft experiences, travelers parties, bitcoin walks. Also try `/find/id--ubud/`, `/find/id--denpasar/` for other areas.
- `https://app.discotech.me/bali/events` — **club guestlists/tickets** (easy, server-rendered list + `?page=2`). Corroborates Beat Bali club nights; event pages at `https://app.discotech.me/events/<id>-<slug>` carry guestlist + ticket tiers.
- `https://www.bandsintown.com/c/bali-indonesia` — **touring concerts** (medium — list scrapes, heavy cookie wall in raw HTML; event links `https://www.bandsintown.com/e/<id>-<slug>` still extractable). Best for cross-checking international headliners.
- `https://www.eventbrite.com/d/indonesia--bali--85672023/events/` — **grassroots layer** (easy list view; also `/events--this-week/`, `/events--this-weekend/`, `/free--events/`). Dinners, yoga, salsa, tech meetups, tribute nights.
- `https://lovebali.baliprov.go.id/article/detail/1768353645163/bali-calendar-of-events-2026:-celebrating-culture,-creativity,-and-quality-tourism-throughout-the-year` — **Bali Govt Calendar of Events 2026** (easy editorial, 56 events month-by-month). Month granularity only — use as watchlist for cultural festivals (e.g. Sep: Taman Ayun Barong Fest, Airshow, Lembongan Surf+Music, Buleleng Fashion Fest; Oct: Nusa Dua Fest, UWRF), then confirm exact dates via search.
- `https://ra.co/events/id/bali` — Resident Advisor electronic listings. **Bot-walled (500 on scrape)** — retry with Playwright stealth only.
...
- **Overlap map (dedup rule):** club headliners → Guideline first (has ticket links), corroborate w/ Discotech + Bandsintown; nightly club parties → Beat Bali first, Discotech second; community/nomad → BaliSquad first, Meetup second; grassroots/workshops → Eventbrite; dining/culture features → NOW! Bali; official culture/sport → Govt CoE watchlist. One event = one entry, prefer the source with the ticket link.
